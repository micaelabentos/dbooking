...
const Booking = () => {
  ...
  const userEmail = user?.email.slice(0,-19);
  ...
  const handleInfoUser = () => {
    ...
    .then((res) => {
      ...
      .then(res => { 
        ...
        "contenido": `<p style="font-weight:600; color: #1f1f1f;">Muchas gracias por realizar una reserva, haz click a continuación para ver tus reservas: </p><a href=${window.location.origin + "/usuario/" + userEmail + "/reserva"} style="color:white; background:#F0572D; width:200px; padding:10px 10px; border-radius: 10px; text-decoration:none; font-weight:600;">ver mis reservas</a>`})
      })
    })
    .catch((error) => console.error("Error", error));
  }
